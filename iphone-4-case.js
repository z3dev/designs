...
function makeDisplay(p) {
    var x = (p.iphone4.ds_rw) + p.offset;
    var y = (p.iphone4.ds_rh) + p.offset;
    var z = p.iphone4.d + ((p.offset + p.thickness)*2);
    var b = CAG.roundedRectangle({center: [0,0], radius: [x,y], roundradius: p.iphone4.ds_rr, resolution: p.resolution});
...
function makeHomeButton(p) {
    ...
    var hb = CSG.cylinder({start: [0,0,0], end: [0,0,z], radius: rr, resolution: p.resolution});
    x = p.iphone4.hb_xoff;
...
function makeRingSilent(p) {
    var r = p.iphone4.ss_rh;
    var x = p.iphone4.ss_rw;
    ...
    b = hull(a,b);
    var z = p.thickness+p.offset;
    b = b.extrude({offset: [0,0,z*2]}).rotateZ(90).rotateY(90);
    x = (-p.iphone4.rw) - z;
    var y = p.iphone4.ss_xoff;
    z = p.iphone4.ss_yoff;
    b = b.translate([x,y,z]);
    return b;
}
function makeVolumeButtons(p) {
    ...
    var vb = CSG.cylinder({start: [0,0,-z], end: [0,0,z], radius: rr, resolution: p.resolution});
...
function makeCameraPort(p) {
    ...
    var bc = CAG.circle({radius: rr, resolution: p.resolution});
    x = p.iphone4.bc_xoff;
    ...
    rr = p.iphone4.bs_rr;
    rr = rr + 1.00; // extra space for access
    var bs = CAG.circle({radius: rr, resolution: p.resolution});
    x = p.iphone4.bs_xoff;
...
function makeFitSlots(p) {
    var x = p.iphone4.rw;
    var y = p.iphone4.rr * 0.25;
    var rr = y * 0.50;
    var z = p.iphone4.rd2 + (p.offset/2);
    var c = CAG.roundedRectangle({center: [0,0], radius: [y,z], roundradius: rr, resolution:16});
    c = c.translate([0,-(p.offset/2)]);
    var c1 = CAG.roundedRectangle({center: [0,0], radius: [y*3,z], roundradius: rr});
    z = (z*2) - (p.offset/2) - rr;
...
function main(p) {
    ...
    var s = use("http://www.z3d.jp/lab/data/lab-000000056/iPhone4_72.json");
    var iphone = CSG.cube();
...
// create the case around the iphone
    ...
    var c = CAG.roundedRectangle({center: [0,0], radius: [x,y], roundradius: rr, resolution: p.resolution});
...
// scale the iPhone to obtain the inside of the case (negative)
    ...
    var ci = iphone.scale([xs,ys,zs]).setColor([1,1,1]);
...
// apply the style
    if (p.style == 'clip') {
        var sl = CSG.cube({center: [0,0,50+p.iphone4.rd2], radius: [200,200,50]});
        c = c.subtract(sl);
