...
function bolt_hole( p ) {
   ...
   p.s_r = p.s_r + fit;
   p.h_r = p.h_r + fit;
   var s = CSG.cylinder({start: [0,0,-p.l], end: [0,0,0], radius: p.s_r, resolution: p.resolution});
   var h = CSG.cylinder({start: [0,0,0], end: [0,0,p.l], radius: p.h_r, resolution: p.resolution});
   return s.union(h);
}
...
// define the rail circle
   ...
   var a = CAG.circle({center: [0,0], radius: p.edge_v_r, resolution: p.resolution});
   var b = CAG.circle({center: [p.offset_h,p.offset_v], radius: p.cap_r, resolution: p.resolution});
   var c = hull(a, b);

   var x = -(p.edge_h_r*2) + (p.flange_h_r*2) + p.arch_r;
   var y = p.arch_r + p.edge_v_r; // TBD calc fudge
...
// TBD add a fudge to align the arch properly
   ...
   var f = CAG.rectangle({center: [x,y], radius: [p.flange_h_r, p.flange_v_r]});
   c = c.union(f);

   x = -p.edge_h_r;
   y = 0;
   var d = CAG.rectangle({center: [x,y], radius: [p.edge_h_r, p.edge_v_r]});
   c = c.subtract(d);
   x = x - p.edge_v_r;
   ...
   b = bolt_hole({s_r: p.screw_s_r, h_r: p.screw_h_r, l: (p.edge_h_r*2), resolution: p.resolution});
