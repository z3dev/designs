function main( p ) {
    ...
    var w = (p.fan_height/2) + (p.thickness*2);
    var h = (p.fan_height/2);
...
// make the adapter for the fan
    ...
    var b = CSG.cube({center: [0,p.thickness*2,p.thickness], radius: [w,h,p.thickness]});
    a = a.union(b);
    ...
    var r = p.fan_m_bolt_z;
    b = CSG.cylinder({start: [0,0,0], end: [0,0,p.thickness*2], radius: r});
    var c = CSG.cube({center: [0,-(p.fan_m_bolt_x+p.fan_thickness)/2,p.thickness], radius: [r,(p.fan_m_bolt_x+p.fan_thickness)/2,p.thickness]});
    b = b.union(c);
    c = CSG.cylinder({start: [0,0,0], end: [0,0,p.thickness*2], radius: p.fan_m_bolt_r});
    b = b.subtract(c);

    var x = (p.fan_height/2) + (p.thickness*2) - (p.thickness*2);
    var y = (p.fan_height/2) + r - 5;
