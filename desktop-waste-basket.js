...
function createTwistCW(p) {
    let o = CAG.circle({radius: p.m_r, center: [p.m_br,0],resolution: p.resolution});

    var step = p.m_r * Math.tan(p.m_a*(Math.PI / 180));
    var steps = p.b_z / step;
    ...
    return slice.solidFromSlices({
            ...
            callback: function(t, sliceno) {
                ...
                var x = slice.xinc * sliceno;
                var z = slice.zinc * sliceno;
                var r = slice.rinc * (sliceno+1);
...
function createTopRim(p) {
    var x = (p.m_d * 1.5) / 2;
    var y = p.b_z * 0.040;
    var z = p.b_td / 2;
    var rr = x * 0.95;
    var o = y - rr;
    var a = CAG.roundedRectangle({center: [0,-o], radius: [x,y], roundradius: rr, resolution: 32});
...
function createBottom(p) {
    ...
    x = r;
    z = (p.m_d * 1.5) / 2;
    var b = CSG.cylinder({start: [0,0,0],end: [0,0,z],radius: x,resolution: 64});
...
function main(p) {
    ...
    var tr = createTopRim(p);
    var br = createBottom(p);

// create the mesh
    var rotation = (p.m_d + p.m_h); // before adjusting
    var circ = Math.PI * 2 * (p.b_bd / 2);
    var rotations = Math.floor((circ / rotation));
    rotation = 360 / rotations; //circ / rotations;
    //console.log(rotation);
    
    var cw_wire = createTwistCW(p).reTesselated();
    var cc_wire = cw_wire.mirroredX();
    var mesh = [];
    while (rotations >= 0) {
        mesh.push(cw_wire.rotateZ(rotation*rotations));
        mesh.push(cc_wire.rotateZ(rotation*rotations));
        rotations--;
    }
