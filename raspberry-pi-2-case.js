...
const createBoard = (p) => {
// blank board
  let b = CAG.roundedRectangle({ center: [0, 0], radius: [p.pi_w_r, p.pi_l_r], roundradius: 3.0, resolution: p.resolution })
  // less mounting holes
  const h = CAG.circle({ center: [0, 0], radius: p.pi_mh_ir, resolution: p.resolution })
...
const makeRow = (holes, p) => {
  let i = 0; const x = 0; let y = 0
  let c = CAG.circle({ center: [x, y], radius: p.case_b_v_h, resolution: p.resolution })
  for (i = 1; i < holes; i++) {
...
// calculate dimensions base on the Rasberry PI board
  ...
  let r = p.pi_c_r + f
  const w = p.pi_w_r + f
  const l = p.pi_l_r + p.pi_usb_e_r + f
  const d = p.case_b_t + p.case_b_h

  let b = CAG.roundedRectangle({ center: [0, 0], radius: [w, l], roundradius: r, resolution: p.resolution })
  b = b.extrude({ offset: [0, 0, d] })
  // less center for electronics and cooling
  const r2 = p.pi_c_r - p.case_b_w_t + f
  const w2 = p.pi_w_r - p.case_b_w_t + f
  const l2 = p.pi_l_r - p.case_b_w_t + f

  let c = CAG.roundedRectangle({ center: [0, 0], radius: [w2, l2], roundradius: r2, resolution: p.resolution })
  ...
  let s = CAG.rectangle({ center: [0, -l], radius: [p.pi_sd_w_r, l] })
  s = s.extrude({ offset: [0, 0, p.pi_sd_d_r * 2] }).translate([0, 0, d - (p.pi_sd_d_r * 2)])
  b = b.subtract(s)
  // plus mounts
  let m = CAG.circle({ center: [0, 0], radius: p.pi_mh_or, resolution: p.resolution })
  ...
  const br = 6 / 2
  m = CAG.rectangle({ center: [0, 0], radius: [br, br] })
  m = m.extrude({ offset: [0, 0, d] })
  x = 0
  y = p.pi_l_r - br - p.pi_usb_e_r
  b = b.union(m.translate([x, y]))
  ...
  // plus clips
  z = p.clip_z
  const ci = CSG.roundedCylinder({ start: [0, 0, -z], end: [0, 0, z], radius: p.clip_r, resolution: p.resolution })
  const cor = ((p.clip_r / 2) + 1) / 2
  let co = CAG.rectangle({ center: [0, 0], radius: [cor, p.clip_r + 1] })
...
// calculate dimensions base on the Rasberry PI board
  ...
  const w = p.pi_w_r + bf + p.case_c_w_t + p.case_c_f
  const l = p.pi_l_r + p.pi_usb_e_r + bf + p.case_c_w_t + p.case_c_f
  const d = p.case_c_t + p.case_c_h + p.case_c_gap + p.case_c_gap_s

  let c = CAG.roundedRectangle({ center: [0, 0], radius: [w, l], roundradius: r, resolution: p.resolution })
  c = c.extrude({ offset: [0, 0, d] })
  // less center for electronics and cooling
  const r2 = p.pi_c_r + bf + p.case_c_f
  const w2 = p.pi_w_r + bf + p.case_c_f
  const l2 = p.pi_l_r + p.pi_usb_e_r + bf + p.case_c_f
  const d2 = p.case_c_h + p.case_c_gap + p.case_c_gap_s

  let s = CAG.roundedRectangle({ center: [0, 0], radius: [w2, l2], roundradius: r2, resolution: p.resolution })
  ...
  const cl = CSG.sphere({ center: [0, 0, 0], radius: p.clip_r, resolution: p.resolution })
  z = (d - p.case_b_t - p.case_b_h - (p.pi_t_r * 2) - p.case_c_f) / 2
  const zhd = p.case_b_t + p.case_b_h + (p.pi_t_r * 2) + p.case_c_f + z
  const xhd = p.pi_mh_ir
  const hd = CSG.cube({ center: [0, 0, 0], radius: [p.pi_mh_ir, p.pi_mh_ir, z] })
  x = -w2
  y = -p.pi_l_r + p.pi_m_y - p.pi_usb_e_r + 58 // located at the mount
  z = p.clip_z
