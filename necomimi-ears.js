...
const pandaEar = () => {
  let cag101 = new CSG.Path2D([[21.693121693121693, -0.25925925925925924]], false)
...
const createNecomimi = (p) => {
  ...
  const b = CAG.roundedRectangle({ center: [0, 0], radius: [x, y], roundradius: rr, resolution: 32 })
  const z = makePath(p)
  let bs = extrudeFromPathA(b, z)
  let s = CSG.cylinder({ start: [0, 0, -5], end: [0, 0, 0], radius: 4, resolution: p.resolution })
...
const createFrame = (p) => {
  const ear = pandaEar().scale([p.scale, p.scale])
  // var frame = ear.rectangularExtrude(4,1,32).center(); // w, h, r
  // frame = frame.union(ear.rectangularExtrude(2,2,32).center());
  let frame = ear.innerToCAG().center([true, true, true]).extrude({ offset: [0, 0, p.wall] })
  ...
  let z = 76
  const rr = x / 2
  let r = CSG.roundedCube({ center: [0, 0, 0], radius: [x, p.wall * 2, z / 2], roundradius: rr, resolution: p.resolution })
...
const createBase = (p) => {
  ...
  const rr = p.necomimi_b_rr
  const c = CAG.roundedRectangle({ center: [0, 0], radius: [x, y], roundradius: rr, resolution: 32 })
  let z = makePath(p)
  const nm = extrudeFromPathA(c, z)
  // and scale to create a shell
  const b = nm.getBounds()
  x = b[1].x - b[0].x
  y = b[1].y - b[0].y
  z = b[1].z - b[0].z
