...
function getLogo (p) {
  let cag101 = new CSG.Path2D([[67.72486772486774, -2.246031746031746]], false)
  ...
  let cag111 = new CSG.Path2D([[36.50793650793651, -22.634920634920636]], false)
  ...
  const b = cag101.getBounds()
  let x = (b[1].x - b[0].x)
  let y = (b[1].y - b[0].y)
  const r = Math.max(x / 2, y / 2)
  let s = CSG.sphere({ radius: r, resolution: p.resolution })
  x = b[0].x + r
  y = b[0].y + r
  s = s.translate([x, y, 0])
  // convert the inside to 3D
  const i = cag111.extrude({ offset: [0, 0, r * 2] }).translate([0, 0, -r])
