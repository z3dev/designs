...
const pandaEar = () => {
  var cag101 = path2.fromPoints({}, [[21.693121693121693,-0.25925925925925924]]);
...
const extrudeFromPathA = (cag, path) => {
  var b = measureBoundingBox(cag);
  var w = b[1][0] - b[0][0];
  var h = b[1][1] - b[0][1];

  const points = path2.toPoints(path);
  var slice0 = slice.fromSides(geom2.toSides(cag));
  slice0.original_x = w;
  slice0.original_y = h;
  ...
  const f = extrudeFromSlices({
    ...
    callback: (p, sliceno, base) => {
      var v = points[sliceno];
      var sx = (base.original_x - (v[0] * 2)) / base.original_x;

      var mS = mat4.fromScaling(mat4.create(), [sx, 1, 1])
      var mT = mat4.fromTranslation(mat4.create(), [0, 0, v[1]])
...
const createNecomimi = (p) => {
  ...
  y = p.necomimi_b_y/2;
  rr = p.necomimi_b_rr;
  var b = roundedRectangle({center: [0,0], size: [x*2,y*2], roundRadius: rr, segments:32});
  var z = makePath(p);
  var bs = extrudeFromPathA(b,z);
  var s = cylinder({height: 10, center: [0, 0, -5], radius: 4, segments: p.segments});
...
const createFrame = (p) => {
  ...
  var frame = extrudeLinear({height: p.wall}, ear);
...
// rotate to Z space and orientate to Z=0
  ...
  var b = measureBoundingBox(frame);
  frame = translate([0,0,-(b[0][2])], frame);
// add ribs for strength
  var x = p.wall/2;
  var z = 76;
  var rr = x/2;
  var r = roundedCuboid({center: [0, 0, 0], size: [x*2,p.wall*2*2,z], roundRadius: rr, segments: 16});
...
const createBase = (p) => {
  ...
  y = p.necomimi_b_y/2;
  rr = p.necomimi_b_rr;
  var c = roundedRectangle({center: [0,0], size: [x*2,y*2], roundRadius: rr, segments:32});
  var z = makePath(p);
  var nm = extrudeFromPathA(c,z);
  // and scale to create a shell
  var b = measureBoundingBox(nm);
  x = b[1][0] - b[0][0];
  y = b[1][1] - b[0][1];
  z = b[1][2] - b[0][2];
...
const main = (p) => {
  ...
  var all = subtract( union(base, frame), core)
  if (p.show === true) {
    all = union(all, nm);
  }
  return all;
