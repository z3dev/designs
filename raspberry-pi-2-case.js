...
const createBoard = (p) => {
  ...
  // less mounting holes
  const h = circle({ radius: p.pi_mh_ir, segments: p.segments })
  ...
  b = colorize(colorNameToRgb('goldenrod'), b)
  return b
}
...
const makeRow = (h, p) => {
  ...
  const x = 0
  let y = 0
  const c = circle({ radius: p.case_b_v_h, segments: p.segments })
  const holes = [c]
  for (i = 1; i < h; i++) {
...
const createVentHoles = (p) => {
  ...
  for (let i = 0; i < p.case_b_v_r; i = i + 2) {
    const row = makeRow(h, p)
    const rowa = translate([x, 0], row)
    const rowb = translate([-x, 0], row)
    rows.push(...rowa)
    rows.push(...rowb)
    h = h - 1
...
const createBase = (p) => {
  ...
  // plus clips
  z = p.clip_z
  const ci = roundedCylinder({ height: z * 2, radius: p.clip_r, roundRadius: p.clip_r * 0.95, segments: p.segments })
  ...
  b = colorize(colorNameToRgb('indianred'), b)
...
const createCover = (p) => {
  ...
  const cl = sphere({ radius: p.clip_r, segments: p.segments })
  ...
  const hd = cuboid({ size: [p.pi_mh_ir * 2, p.pi_mh_ir * 2, z * 2] })
  x = -w2
  y = -p.pi_l_r + p.pi_m_y - p.pi_usb_e_r + 58 // located at the mount
  z = p.clip_z
  ...
  c = colorize(colorNameToRgb('forestgreen'), c)
